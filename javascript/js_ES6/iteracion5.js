...
const agesAdult = ages.filter((age) => age > 18);
...
const agesPar = ages2.filter((age) => age % 2 == 0);
console.log(agesPar);
...
const bestGame = streamers.filter(
...
const uStreamers = streamers2.filter((user) => user.name.includes("u"));
...
const streamersLegends = streamers2.filter((user) =>
  user.gameMorePlayed.includes("Legends")
);
for (const user of streamersLegends) {
  if (user.age > 35) {
